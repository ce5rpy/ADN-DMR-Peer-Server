"""BridgesLegacyView: subscription store → pickle BRIDGE_SND shim."""
...
from adn_server.application.subscription.bridges_legacy_view import BridgesLegacyView
...
def test_generate_matches_export_bridges():
    store = _sample_store()
    view = BridgesLegacyView(store)
    now = 1_700_000_000.0
    assert view.generate(now=now) == export_bridges(store, now=now)


def test_bridge_snd_frame_is_pickle_protocol_2():
    store = _sample_store()
    frame = encode_bridge_snd_frame(BridgesLegacyView(store).generate())
