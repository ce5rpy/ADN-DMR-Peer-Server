# ADN DMR Peer Server - tests application routing table export
#
...
from adn_server.application.subscription.routing_table_export import export_routing_table, subscription_to_legacy_row
...
def test_export_routing_table_groups_by_table_key():
    ...
    store.upsert(
        Subscription(
            ...
            policy=ActivationPolicy.OPENBRIDGE_STAT,
            state=SubscriptionState(phase=SubscriptionPhase.ACTIVE),
            relay_table_key="730444",
        )
    )
    bridges = export_routing_table(store)
